...
export default function SwapPage() {
  ...
  useEffect(() => {
    const fetchInitialQuote = async (): Promise<void> => {
      try {
        ...
        if (err instanceof Error) {
          setQuoteError(
            `Failed to load quote. Please refresh the page. Error: ${err.message}`,
          );
        } else {
          setQuoteError(
            "Failed to load quote. Please refresh the page. An unexpected error occurred.",
          );
        }
      }
    };
  ...
  const getSwapTransaction = async (
    ...
    try {
      ...
      if (err instanceof Error) {
        toast.error(
          `Failed to create swap transaction. Please try again. Error: ${err.message}`,
        );
      } else {
        toast.error(
          "Failed to create swap transaction. Please try again. An unexpected error occurred.",
        );
      }
      throw err;
    }
  ...
  const executeSwap = async () => {
    ...
    try {
      ...
      try {
        ...
        if (err instanceof Error) {
          toast.error(
            `Failed to process transaction data. Please try again. Error: ${err.message}`,
          );
        } else {
          toast.error(
            "Failed to process transaction data. Please try again. An unexpected error occurred.",
          );
        }
        setLoading(false);
        return;
      ...
      if (
        ...
        toast.warn(
          "Transaction submitted but confirmation timed out. Check your wallet for status.",
        );
      } else {
        setSuccess(true);
                  ...
                  getQuote()
                    ...
                    .catch((err: unknown) => {
                      if (err instanceof Error) {
                        ...
                        setQuoteError(
                          "Failed to load quote: An unexpected error occurred.",
                        );
                      }
                    })
                }
              ...
              : !quoteData
                ...
                : loading
                  ...
                  : success
                    ...
                    : `Buy ${buyAmount / 1000000} $WORK for ${quoteData ? formatSolAmount(quoteData.inAmount) + " SOL" : "..."}`}
          </button>

          {success && (
            <div className="mt-6 p-4 bg-green-100 border border-green-300 rounded-lg">
              <p className="font-medium mb-2">
                🎉 Congratulations! You&apos;ve successfully purchased 200,000
                WORK tokens.
              </p>
              {txId && (
                <a
